...
2. Download `ZCompare-0.1.0-win-x64-setup.exe` and `SHA256SUMS.txt`.
3. Verify the SHA-256 value:

   ```powershell
   Get-FileHash .\ZCompare-0.1.0-win-x64-setup.exe -Algorithm SHA256
...
The installer is not code-signed in v0.1.0. SmartScreen may warn about an unrecognized app. Continue through **More info → Run anyway** only after verifying the official source and checksum. Never disable SmartScreen for installation.
...
2. Download and verify `ZCompare-0.1.0-win-x64-portable-fdd.zip`.
