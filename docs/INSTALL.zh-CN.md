...
2. 下载 `ZCompare-0.1.0-win-x64-setup.exe` 和 `SHA256SUMS.txt`。
3. 校验 SHA-256：
...
v0.1.0 暂未购买代码签名，因此 SmartScreen 可能提示“无法识别的应用”。只有在确认来源是官方仓库且 SHA-256 一致后，才按 **“更多信息”→“仍要运行”**；不要关闭 SmartScreen。
...
2. 下载并校验 `ZCompare-0.1.0-win-x64-portable-fdd.zip`。
3. 解压到一个新文件夹。
...
程序不会把工作簿、路径、报告或设置发送到服务器。更新检查只访问 `api.github.com`，发送标准 HTTP 头和已安装版本。最近路径与更新缓存保存在 `%LocalAppData%\ZCompare`。v0.1.0 不生成诊断日志文件。
